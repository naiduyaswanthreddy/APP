...
const ProfileBasic = ({ userData: propUserData = {}, isAdminView, onUserDataChange }) => {
  ...
  const fetchApplicationStats = async () => {
    ...
    if (user) {
      try {
        ...
        // Backend stores the student reference as "student_id"
        ...
        const eligibleJobs = allJobs.filter(job => {
          ...
          // Job schema uses maxCurrentArrears; fall back safely if absent
          const maxCurrentArrears = job.maxCurrentArrears ?? job.maxArrears; // support older field name if any
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (isAdminView && propUserData && propUserData.id) {
        ...
        const user = auth.currentUser;
        if (!user) {
          toast.error("User not authenticated. Please log in.", { toastId: 'profile-auth-error' });
          setIsSaving(false);
          return;
        }
      ...
      toast.error("Failed to update profile. Please try again.", { toastId: 'profile-error' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleUploadProfilePhoto = () => {
    ...
    if (photoUrl) {
      ...
      toast.success("Profile photo updated!", { toastId: 'photo-success' });
    }
  };
    ...
    <div className="pb-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        <StatCard 
          icon={<Target />} 
          label="Eligible Jobs" 
          value={userData.eligibleJobs || 0} 
          bgColor="bg-blue-50" 
        />
        <StatCard 
          icon={<FileText />} 
          label="Applied Jobs" 
          value={userData.appliedJobs || 0} 
          bgColor="bg-green-50" 
        />
        <StatCard 
          icon={<AlertTriangle />} 
          label="Not Applied" 
          value={userData.notAppliedEligible || 0} 
          bgColor="bg-yellow-50" 
        />
        <StatCard 
          icon={<Award />} 
          label="Offers" 
          value={userData.offersReceived || 0} 
          bgColor="bg-purple-50" 
        />
        <StatCard 
          icon={<Calendar />} 
          label="Placement Cycle" 
          value={userData.placementCycle || "N/A"} 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl shadow p-6 bg-green-50 border border-gray-200 mb-6">
          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            <div>
              ...
              {isEditing ? (
                <input type="email" className="text-lg text-gray-800 border rounded px-2 py-1 w-full" value={editData.fatherEmail} onChange={e => setEditData({ ...editData, fatherEmail: e.target.value })} />
              ) : (
            ...
            <div className="col-span-2">
              ...
              {isEditing ? (
                <input type="text" className="text-lg text-gray-800 border rounded px-2 py-1 w-full" value={editData.fatherOfficeAddress} onChange={e => setEditData({ ...editData, fatherOfficeAddress: e.target.value })} />
              ) : (
            ...
            <div>
              ...
              {isEditing ? (
                <input type="email" className="text-lg text-gray-800 border rounded px-2 py-1 w-full" value={editData.motherEmail} onChange={e => setEditData({ ...editData, motherEmail: e.target.value })} />
              ) : (
            ...
            <div className="col-span-2">
              ...
              {isEditing ? (
                <input type="text" className="text-lg text-gray-800 border rounded px-2 py-1 w-full" value={editData.motherOfficeAddress} onChange={e => setEditData({ ...editData, motherOfficeAddress: e.target.value })} />
              ) : (
        ...
        <div className="rounded-xl shadow p-6 bg-green-50 border border-gray-200 mb-6">
          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            <div className="col-span-2">
              ...
              {isEditing ? (
                <input type="text" className="text-lg text-gray-800 border rounded px-2 py-1 w-full" value={editData.address} onChange={e => setEditData({ ...editData, address: e.target.value })} />
              ) : (
        ...
        <div className={`rounded-xl shadow p-6 border border-gray-200 mb-6 ${propUserData.freezed?.active ? 'bg-red-50' : 'bg-green-50'}`}>
          <h3 className="text-lg font-semibold flex items-center gap-2 mb-4 text-gray-800 border-b pb-2">
            ...
            Freeze Status
            {isAdminView && propUserData.freezed?.active && (
              <button
                onClick={async () => {
                  if (window.confirm('Are you sure you want to unfreeze this student?')) {
                    try {
                      const reason = prompt('Please provide a reason for unfreezing:');
                      if (reason && reason.trim()) {
                        const studentPayload = [{
                          id: propUserData.uid || propUserData.id,
                          rollNumber: propUserData.rollNumber || 'UNKNOWN',
                          batch: propUserData.batch || '',
                          department: propUserData.department || '',
                          currentFreeze: {
                            active: true,
                            freezeHistory: propUserData.freezeHistory || []
                          }
                        }];
                        ...
                        if (onUserDataChange) {
                          const updatedDoc = await getDoc(doc(db, 'students', propUserData.uid || propUserData.id));
                          onUserDataChange(updatedDoc.data());
                        }
                      }
                    } catch (error) {
          ...
          {propUserData.freezed?.active ? (
            ...
            <div className="bg-green-100 border border-green-300 rounded-lg p-4">
              ...
              <p className="text-green-700 text-sm mt-1">Student can apply for jobs and participate in placement activities.</p>
            </div>
          )}
